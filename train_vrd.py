...
from dataloader.relationship import pre_VRD, VRD
...
def parse_args():
    ...
    parser.add_argument('--save_dir', dest='save_dir',
                        help='directory to save models', default="models/relation",
                        type=str)
...
if __name__ == '__main__':
    ...
    imdb_train = 'data/cache/' + args.imdb_name + '.pkl'
    ...
    train_dataset = VRD(data_opts, train_set, cfg.MAX_NUM_GT_BOXES, 'train')
    ...
    if args.net == 'vgg16_deform':
        fasterRCNN = vgg16(train_dataset.object_classes, pretrained=True, class_agnostic=args.class_agnostic)
    elif args.net == 'res101_deform':
        fasterRCNN = resnet(train_dataset.object_classes, 101, pretrained=True, class_agnostic=args.class_agnostic)
    elif args.net == 'res50':
    ...
    for key, value in dict(fasterRCNN.named_parameters()).items():
        if value.requires_grad:
            if 'deform_fc.weight' in key:
                params += [{'params': [value], 'lr': lr * 0.01, 'weight_decay': cfg.TRAIN.WEIGHT_DECAY}]

            elif 'deform_fc.bias' in key:
                params += [{'params': [value], 'lr': lr * 0.01 * (cfg.TRAIN.DOUBLE_BIAS + 1), \
